...
const initializeDbAndServer = async () => {
  const uri =
    "mongodb+srv://srinu879033:6304403720@cluster0.mhdjq.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
...
const authenticateToken = (request, response, next) => {
  ...
  if (jwtToken === undefined) {
    ...
    jwt.verify(jwtToken, "knock-knock", async (error, payload) => {
      if (error) {
        response.status(401);
        response.send("Invalid JWT Token");
      } else {
...
const apiServices = (client) => {
  ...
  app.post("/register/", async (request, response) => {
    ...
    if (result.length === 0) {
      ...
      let fromMail = "fashionfit720@gmail.com";
      let toMail = mailId;
  ...
  app.post("/login/", async (request, response) => {
    ...
    if (result1.length === 0) {
      ...
      if (isPasswordMatched == true) {
        ...
        const jwtToken = jwt.sign(payload, "knock-knock");
